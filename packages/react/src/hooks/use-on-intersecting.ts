...
type IntersectCallback = (
  props: Array<IntersectionObserverEntry | null>,
  didResize?: boolean,
) => DisposeFn | void | null;
...
export function useIsIntersecting<Ref extends HTMLRef | HTMLRef[]>(
  ...
  return (Array.isArray(refs) ? values : values[0]) as Ref extends any[] ? boolean[] : boolean;
}
...
export function useOnIntersecting<Ref extends HTMLRef | HTMLRef[]>(
  ...
  useEffect(() => {
    ...
    let currentEntries: Array<IntersectionObserverEntry | null> = [];
    const options = {
      threshold,
      root,
      rootMargin,
