...
import { ToastAnnounce } from '../announce/toast-announce';
import { Collection, useToastProviderContext } from '../provider/toast-provider';
...
const ToastImpl = forwardRef<TamaguiElement, ToastImplProps>(
  (props: ToastScopedProps<ToastImplProps>, forwardedRef) => {
    const {
      __scopeToast,
      type = 'foreground',
    ...
    useEffect(() => {
      ...
      if (viewport) {
        viewport.addEventListener(TOAST_VIEWPORT_PAUSE, handlePause);
        viewport.addEventListener(TOAST_VIEWPORT_RESUME, handleResume);
        return () => {
          viewport.removeEventListener(TOAST_VIEWPORT_PAUSE, handlePause);
          viewport.removeEventListener(TOAST_VIEWPORT_RESUME, handleResume);
        };
      }
...
export { ToastImpl, useToastInteractiveContext };
