...
export type InputVariantSpreadExtras<T extends StylableComponent> = VariantSpreadExtras<GetProps<T> & InputContextType>;
...
type ExcludePropsUnion = 'secureTextEntry' | 'onChangeText' | 'editable' | 'keyboardType' | 'placeholderTextColor' | 'selectionColor' | 'numberOfLines' | 'multiline';
type DetailedInputProps = Omit<ComponentProps<'input'>, 'className' | 'children' | 'value' | 'size' | ExcludePropsUnion>;
export type InputElementBaseProps = GetProps<typeof InputElement>;
export type InputElementProps = DetailedInputProps & InputElementBaseProps & Omit<TextInputProps, ExcludePropsUnion> & {
