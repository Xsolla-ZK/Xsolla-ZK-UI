import * as badgeThemes from './components-theme/badge';
import * as buttonThemes from './components-theme/button';
import * as cellThemes from './components-theme/cell';
import * as checkboxThemes from './components-theme/checkbox';
import * as chipsThemes from './components-theme/chips';
import * as dialogThemes from './components-theme/dialog';
import * as flexButtonThemes from './components-theme/flex-button';
import * as inlineInputThemes from './components-theme/inline-input';
import * as inputThemes from './components-theme/input';
import * as listThemes from './components-theme/list';
import * as loaderThemes from './components-theme/loader';
import * as pimpleThemes from './components-theme/pimple';
import * as progressBarThemes from './components-theme/progress-bar';
import * as radioGroupThemes from './components-theme/radio-group';
import * as richIconThemes from './components-theme/rich-icon';
import * as segmentedControlThemes from './components-theme/segmented-control';
import * as sheetThemes from './components-theme/sheet';
import * as sliderThemes from './components-theme/slider';
import * as switchThemes from './components-theme/switch';
import * as tabBarThemes from './components-theme/tab-bar';
import * as tabsThemes from './components-theme/tabs';
import * as toastThemes from './components-theme/toast';

const componentsTheme = {
  ...buttonThemes,
  ...badgeThemes,
  ...cellThemes,
  ...checkboxThemes,
  ...chipsThemes,
  ...loaderThemes,
  ...pimpleThemes,
  ...inputThemes,
  ...chipsThemes,
  ...flexButtonThemes,
  ...checkboxThemes,
  ...radioGroupThemes,
  ...richIconThemes,
  ...tabsThemes,
  ...switchThemes,
  ...sliderThemes,
  ...tabBarThemes,
  ...sheetThemes,
  ...dialogThemes,
  ...progressBarThemes,
  ...segmentedControlThemes,
  ...inlineInputThemes,
  ...toastThemes,
  ...cellThemes,
  ...listThemes,
};

export { componentsTheme };
